...
- [x] Create `backend/app/models/route.py` — Route model (added to optimization.py)
- [x] Create Alembic migration for route table
- [x] Create `backend/app/services/routing.py` — Route optimizer:
...
## Tests
- [x] `test_distance_matrix` — OSRM returns valid matrix
- [x] `test_cvrp_solver` — OR-Tools produces valid routes
- [x] `test_route_respects_capacity` — No route exceeds vehicle capacity
- [x] `test_route_respects_duration` — No route exceeds max duration
- [x] `test_polyline_generation` — Routes have valid encoded polylines
- [x] `test_stop_order_optimized` — Stops ordered to minimize distance
- [x] `test_eta_calculation` — ETA computed per stop
- [x] `test_two_leg_model` — Access and main legs separate
- [x] `test_sequential_fallback` — Fallback assignment works
- [x] `test_per_site_routing` — Routes scoped to site
