...
- [ ] Create `backend/app/services/meeting_zones.py` — Meeting zone optimizer:
...
- [ ] Implement employee-to-gathering-point assignment:
...
## Tests
- [ ] `test_centroid_snap_to_road` — Centroid snapped to nearest road
- [ ] `test_walking_distance_constraint` — All employees within max distance
- [ ] `test_pmr_accessibility` — PMR clusters get accessible zones
- [ ] `test_employee_assignment` — Each employee assigned to one zone
- [ ] `test_access_leg_calculation` — Walking distance/time computed
- [ ] `test_osrm_nearest` — OSRM nearest service integration
- [ ] `test_osrm_route` — OSRM route service integration
