...
## Tasks
- [x] Create ContentListPage with table displaying title, type, status, published date, audience, and engagement
- [x] Add filters for content type and status on the list page
- [x] Create ContentCreatePage with title input, rich text editor for body, type selector, media upload, audience targeting (sites, departments, shifts), schedule fields (publish/expiry dates), and preview button
- [x] Create ContentEditPage with pre-filled form from existing content data
- [x] Create ContentDetailPage with read-only view and engagement metrics display
- [x] Wire all pages to the Content CRUD API endpoints
...
## Tests
- [x] Test ContentCreatePage form rendering with all fields
- [x] Test audience targeting UI (site, department, shift selectors)
- [x] Test ContentEditPage loads and pre-fills data correctly
- [x] Test ContentListPage renders table with filters
- [x] Test ContentDetailPage displays read-only content and engagement metrics
- [x] Test preview button functionality

## Test Results
- Tests written: 26
- Tests passing: 26
- Tests failing: 0
- Coverage: 6 test files across pages and components
