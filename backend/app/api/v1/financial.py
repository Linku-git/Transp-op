...
# ---------------------------------------------------------------------------
# POST /financial/export/daf — ERP-compatible export
# ---------------------------------------------------------------------------


@router.post("/export/daf")
async def daf_export(
    body: DAFExportRequest,
    current_user: User = Depends(require_role("admin", "daf")),
) -> Response:
    """Generate ERP-compatible accounting export (CSV or XML)."""
    entries: list[dict] = []
    if body.tco_data:
        entries.extend(build_tco_entries(body.tco_data))
    if body.roi_data:
        entries.extend(build_roi_entries(body.roi_data))

    if not entries:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No data provided for export (tco_data or roi_data required)",
        )

    if body.output_format == "xml":
        content = generate_daf_xml(entries, body.erp_format)
        media_type = "application/xml"
        filename = f"daf_export_{body.erp_format}.xml"
    else:
        content = generate_daf_csv(entries, body.erp_format)
        media_type = "text/csv"
        filename = f"daf_export_{body.erp_format}.csv"

    logger.info(
        "DAF export (%s/%s) with %d entries by user %s",
        body.erp_format,
        body.output_format,
        len(entries),
        current_user.id,
...
# ---------------------------------------------------------------------------
# POST /export/financial/tco — TCO report (PDF or Excel)
# ---------------------------------------------------------------------------


@router.post("/export/tco")
async def export_tco_report(
    body: FinancialReportRequest,
    current_user: User = Depends(require_role("admin", "daf", "drh")),
) -> Response:
    """Generate TCO report as PDF or Excel."""
    if body.report_format == "excel":
        content = generate_tco_excel(body.data)
        media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        filename = "tco_report.xlsx"
    else:
        content = generate_tco_pdf(body.data)
        media_type = "application/pdf"
        filename = "tco_report.pdf"

    logger.info("TCO report (%s) generated by user %s", body.report_format, current_user.id)
    return Response(
        content=content,
        media_type=media_type,
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
...
# ---------------------------------------------------------------------------
# POST /export/financial/roi — ROI report (PDF or Excel)
# ---------------------------------------------------------------------------


@router.post("/export/roi")
async def export_roi_report(
    ...
    if body.report_format == "excel":
        content = generate_roi_excel(body.data)
        media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        filename = "roi_report.xlsx"
    else:
        content = generate_roi_pdf(body.data)
        media_type = "application/pdf"
        filename = "roi_report.pdf"

    logger.info("ROI report (%s) generated by user %s", body.report_format, current_user.id)
    return Response(
        content=content,
        media_type=media_type,
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
