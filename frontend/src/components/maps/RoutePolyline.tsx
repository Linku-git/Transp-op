...
const ROUTE_COLORS = [
  '#0058be', '#d97706', '#7c3aed', '#dc2626',
  '#495e8a', '#059669', '#924700', '#ca8a04',
];
...
export function RoutePolyline({ route, index, isSelected = false }: RoutePolylineProps) {
        ...
        <InfoWindow position={infoPos} onCloseClick={() => setInfoPos(null)}>
          <div className="font-sans text-sm min-w-[180px] p-1">
            ...
            {route.total_distance_km != null && (
              <p className="text-on-surface-variant">Distance: {Number(route.total_distance_km).toFixed(1)} km</p>
            )}
            {route.total_time_minutes != null && (
              <p className="text-on-surface-variant">Durée: {Number(route.total_time_minutes).toFixed(0)} min</p>
            )}
          </div>
        </InfoWindow>
      )}
